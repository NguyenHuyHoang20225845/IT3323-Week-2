...
void skipComment() {
  ...
  while (currentChar != EOF) {
    if (CHCODE(currentChar) == CHAR_TIMES) {
      ...
      // else continue scanning (maybe multiple '*' in a row)
    } else {
      readChar();
    }
  }
  // if we reach here, EOF reached before closing "*)"
...
Token* readIdentKeyword(void) {
  ...
  while (currentChar != EOF && (CHCODE(currentChar) == CHAR_LETTER || CHCODE(currentChar) == CHAR_DIGIT)) {
    if (count < MAX_IDENT_LEN) {
      ...
      // still need to consume remaining identifier characters
      // but we only store up to MAX_IDENT_LEN
      count++; // still increment to know it's longer
    }
    readChar();
  }

  if (count > MAX_IDENT_LEN) {
    // truncate stored string and report error
  ...
  // check if it's a keyword (compare using uppercase copy)
  // checkKeyword expects a C-string; many implementations expect uppercase
  // so we pass tmp (uppercase copy)
  TokenType kt = checkKeyword(tmp);
  if (kt != TK_IDENT) token->tokenType = kt;
  return token;
}
